...
//	4. Se debe poder consultar si un número está dentro de un rango.
	public boolean contiene(double numero) {
		boolean dentroInicio = this.isAbiertoIzquierda() ? numero > this.getLimiteInferior() : numero >= this.getLimiteInferior();
        boolean dentroFin = this.isAbiertoDerecha() ? numero < this.getLimiteSuperior() : numero <= this.getLimiteSuperior();
...
//	5. Se debe poder consultar si un rango está dentro de un rango.
	public boolean contiene(Rango rango) {
		boolean dentroInicio = this.isAbiertoIzquierda() ? rango.getLimiteInferior() > this.getLimiteInferior(): rango.getLimiteInferior() >= this.getLimiteInferior();
        boolean dentroFin = this.isAbiertoDerecha() ? rango.getLimiteSuperior() < this.getLimiteSuperior() : rango.getLimiteSuperior() <= this.getLimiteSuperior();
...
//	6. Se debe poder consultar si hay intersección entre dos rangos.
	public boolean hayInterseccion(Rango rango) {
		boolean dentroInicio = this.isAbiertoIzquierda() ? rango.getLimiteSuperior() > this.getLimiteInferior(): rango.getLimiteSuperior() >= this.getLimiteInferior();
        boolean dentroFin = this.isAbiertoDerecha() ? rango.getLimiteInferior() < this.getLimiteSuperior() : rango.getLimiteInferior() <= this.getLimiteSuperior();
